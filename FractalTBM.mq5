...
// ─── TP/SL/BE тоолуур (comment-р ялгана) ───
int res_fractal_tp = 0, res_fractal_sl = 0, res_fractal_be = 0;
int res_tbm_tp = 0, res_tbm_sl = 0, res_tbm_be = 0;
int res_combined_tp = 0, res_combined_sl = 0, res_combined_be = 0;
...
void OnDeinit(const int reason)
{
   ...
   Print("══════ ҮР ДҮН (TP / SL / BE) ══════");
   Print("  FRACTAL:  TP:", res_fractal_tp, " | SL:", res_fractal_sl, " | BE:", res_fractal_be,
         " | Win%:", (res_fractal_tp + res_fractal_sl + res_fractal_be) > 0 ?
         IntegerToString((int)MathRound(res_fractal_tp * 100.0 / (res_fractal_tp + res_fractal_sl + res_fractal_be))) + "%" : "N/A");
   Print("  TBM:      TP:", res_tbm_tp, " | SL:", res_tbm_sl, " | BE:", res_tbm_be,
         " | Win%:", (res_tbm_tp + res_tbm_sl + res_tbm_be) > 0 ?
         IntegerToString((int)MathRound(res_tbm_tp * 100.0 / (res_tbm_tp + res_tbm_sl + res_tbm_be))) + "%" : "N/A");
   Print("  COMBINED: TP:", res_combined_tp, " | SL:", res_combined_sl, " | BE:", res_combined_be,
         " | Win%:", (res_combined_tp + res_combined_sl + res_combined_be) > 0 ?
         IntegerToString((int)MathRound(res_combined_tp * 100.0 / (res_combined_tp + res_combined_sl + res_combined_be))) + "%" : "N/A");
   Print("── НИЙТ ──");
   ...
   int total_tp = res_fractal_tp + res_tbm_tp + res_combined_tp;
   int total_sl = res_fractal_sl + res_tbm_sl + res_combined_sl;
   int total_be = res_fractal_be + res_tbm_be + res_combined_be;
   Print("  Нийт TP:", total_tp, " | SL:", total_sl, " | BE:", total_be);
   Print("═══════════════════════════════════════════");
}
...
{
   // Позиц хаагдсан үед TP/SL/BE тоолох
   ...
   if(HistoryDealSelect(deal_ticket))
   {
      ...
      if(deal_entry != DEAL_ENTRY_OUT) return;  // зөвхөн хаах deal
      ...
      // Аль позицын comment-р ялгах — parent position-н comment хэрэгтэй
      // Deal comment-д "[tp]", "[sl]" байвал тодорхой
      bool is_tp = (StringFind(comment, "[tp]") >= 0) || (profit > 0 && StringFind(comment, "[sl]") < 0);
      bool is_sl = (StringFind(comment, "[sl]") >= 0);
      bool is_be = is_sl && MathAbs(profit) < 5.0;  // SL hit but near zero = BE

      // Position comment-с төрлийг олох
      // Deal-н position ID-р хайх
      ulong pos_id = HistoryDealGetInteger(deal_ticket, DEAL_POSITION_ID);
      string pos_comment = "";

      // Түүхээс position-н нээсэн deal-г олж comment авах
      if(HistorySelectByPosition(pos_id))
      {
         for(int i = 0; i < HistoryDealsTotal(); i++)
         {
            ...
            {
               pos_comment = HistoryDealGetString(d, DEAL_COMMENT);
               break;
            }
         }
      }

      if(StringFind(pos_comment, "COMBINED") >= 0)
      {
         if(is_tp) res_combined_tp++;
         else if(is_be) res_combined_be++;
         else if(is_sl) res_combined_sl++;
      }
      else if(StringFind(pos_comment, "TBM") >= 0)
      {
         if(is_tp) res_tbm_tp++;
         else if(is_be) res_tbm_be++;
         else if(is_sl) res_tbm_sl++;
      }
      else if(StringFind(pos_comment, "FRACTAL") >= 0)
      {
         if(is_tp) res_fractal_tp++;
         else if(is_be) res_fractal_be++;
         else if(is_sl) res_fractal_sl++;
      }
   }
}
...
void CheckEntry(double atr_val)
{
   ...
   // Pine Script-тэй ижил шүүлтүүр
   // ═══════════════════════════════════════════
   if(mom_count <= 0) return;

   // Pine: bar_index - t_last_bar > 5 — сүүлийн TBM entry-с 5+ бар өнгөрсөн байх
   if(cur_bar - t_last_bar <= 5) return;
   ...
   for(int i = 0; i < mom_count; i++)
   {
      ...
      // Pine: too_close = t_last_bar < 20 bars && distance < ATR*2
      bool too_close = (t_last_price > 0 && cur_bar - t_last_bar < 20 && MathAbs(bid_price - t_last_price) < atr_val * 2);
      ...
      // Pine Script-тэй ижил: BAR CLOSE дээр шалгах (bar 1 = сүүлийн хаагдсан бар)
      double bar1_low = GetLow(1);
      double bar1_high = GetHigh(1);
      double bar1_close = GetClose(1);
      double bar1_open = GetOpen(1);

      // Зөвхөн conf (★) эсвэл combined дээр орно — plain TBM skip
      bool is_combined = false;
      if(!conf)
      {
         is_combined = f_pb_active && ((fd == -1 && f_disp_dir == 1) || (fd == 1 && f_disp_dir == -1));
         if(!is_combined) continue;  // conf биш, combined биш → skip
      }

      // Bull TBM entry: Pine — low <= f618 AND close > f618 AND close > open
      if(fd == -1 && bar1_low <= f618 && bar1_close > f618 && bar1_close > bar1_open)
      {
         if(open_count >= InpMaxPositions) break;

         is_combined = f_pb_active && f_disp_dir == 1;
         double entry_price = ask_price;
         double sl = entry_price - InpSL_Pips * pip;
         double tp = entry_price + InpTP_Pips * pip;
         string comment = is_combined ? "COMBINED_BUY" : "TBM_BUY_CONF";
         double lot = CalcLotSize();

         if(trade.Buy(lot, _Symbol, 0, sl, tp, comment))
         {
            if(is_combined) { cnt_combined_buy++; Print("🟢 ", comment, " #", cnt_combined_buy); }
            else { cnt_tbm_conf_buy++; Print("🟢 ", comment, " #", cnt_tbm_conf_buy); }
      ...
      // Bear TBM entry: Pine — high >= f618 AND close < f618 AND close < open
      if(fd == 1 && bar1_high >= f618 && bar1_close < f618 && bar1_close < bar1_open)
      {
         if(open_count >= InpMaxPositions) break;
         ...
         double sl = entry_price + InpSL_Pips * pip;
         double tp = entry_price - InpTP_Pips * pip;
         string comment = is_combined ? "COMBINED_SELL" : "TBM_SELL_CONF";
         double lot = CalcLotSize();

         if(trade.Sell(lot, _Symbol, 0, sl, tp, comment))
         {
            if(is_combined) { cnt_combined_sell++; Print("🔴 ", comment, " #", cnt_combined_sell); }
            else { cnt_tbm_conf_sell++; Print("🔴 ", comment, " #", cnt_tbm_conf_sell); }
            Print("   Lot: ", lot, " | Entry: ", entry_price, " | SL: ", sl, " | TP: ", tp);
            if(is_combined) f_active_pattern = 1;
            t_last_bar = cur_bar;
            t_last_price = entry_price;
