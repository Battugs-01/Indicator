...
void CheckEntry(double atr_val)
{
   ...
   // ═══════════════════════════════════════════
   // FRACTAL ENTRY — CHOCH + Grade систем
   // ═══════════════════════════════════════════
   if(f_pb_active)
   {
      ...
      // ── Entry Grade тодорхойлох ──
      bool htf_aligned = (f_disp_dir == 1 && htf_trend == 1) || (f_disp_dir == -1 && htf_trend == -1);

      int grade = 0;
      if(liq_sweep && htf_finalize && (entry_trigger_bull || entry_trigger_bear)) grade = 1;       // A.1
      else if(liq_sweep && htf_aligned && (entry_trigger_bull || entry_trigger_bear)) grade = 2;   // A
      else if((entry_trigger_bull || entry_trigger_bear)) grade = 3;                                // Pot.A.1

      string grade_name = grade == 1 ? "A.1" : grade == 2 ? "A" : grade == 3 ? "Pot.A.1" : "NONE";

      bool entry_bull = (entry_trigger_bull && grade >= 1 && grade <= 3);
      bool entry_bear = (entry_trigger_bear && grade >= 1 && grade <= 3);
   ...
   // ═══════════════════════════════════════════
   // TBM ENTRY: Түр унтраасан — зөвхөн Fractal ажиллана
   return;  // TBM entry skip
